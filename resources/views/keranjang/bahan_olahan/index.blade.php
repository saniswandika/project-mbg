            ...
            <tbody>
                ...
                @endphp 
                    <tr>
                        ...
                        <td>
                            @if($item->status == NULL)
                                <form action="{{ route('bahan_olahan.hapus_keranjang', $item->id) }}" method="POST" class="d-inline" onsubmit="return confirmDelete()">
                                    @csrf
                                    @method('DELETE')
                            ...
                            @if($admin && ($role == 'admin' || $role == 'superadmin'))
                                <form action="{{ route('bahan_olahan.status_change') }}" method="POST" class="d-inline">
                                    @csrf
                                ...
                                <form action="{{ route('bahan_olahan.status_change') }}" method="POST" class="d-inline" onsubmit="return confirmDelete()">
                                    @csrf
                                    <input type="hidden" name="id_user" value="{{ Auth::id() }}">
                                    <input type="hidden" name="id" value="{{ $item->id }}">
...
@if($cartItems->isNotEmpty())
    ...
    <form action="{{ route('bahan_olahan.ajukan_pengambilan') }}" method="POST">
        @csrf
        <input type="hidden" name="id_user" value="{{ Auth::id() }}">
